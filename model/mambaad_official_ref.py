...
class LSSModule(nn.Module):
	...
	def forward(self, input: torch.Tensor):
		...
		out_77 = self.conv77(out_ssm.permute(0, 3, 1, 2).contiguous())
		output = torch.cat((out_33, out_55, out_77), dim=1)
...
def mambaad_official_ref(pretrained=False, **kwargs):
	model = MAMBAAD(**kwargs)
	return model

@MODEL.register_module
def mambaadhcs57c1(pretrained=False, **kwargs):
	model = MAMBAAD(**kwargs)
	return model
