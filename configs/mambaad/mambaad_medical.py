...
class cfg(cfg_common, cfg_dataset_default, cfg_model_mambaad):
	...
	def __init__(self):
		...
		# ==> loss
		self.loss.lambda_l1 = 0.005
		self.loss.adaptive_mc_weight_start = 0.01
		self.loss.adaptive_mc_weight_end = 1.0
		self.loss.adaptive_mc_warmup_epochs = 10
		self.loss.loss_terms = [
			dict(type='L2Loss', name='pixel', lam=5.0),
			dict(type='AdaptiveMCLoss', name='adaptive_mc', m_base=0.2, alpha=0.3),
		]
		...
		self.logging.log_terms_train = [
			...
			dict(name='adaptive_mc', suffixes=[''], fmt=':>5.3f', add_name='avg'),
